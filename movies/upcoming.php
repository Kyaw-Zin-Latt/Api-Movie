...
if (isset($_GET['page']) && (empty($_POST['sort_by']) && empty($_GET['sort_by']) && empty($_POST['start']) && empty($_POST['end']) && empty($_GET['start']) && empty($_GET['end']) && empty($_POST['genres']) && empty($_GET ['genres']) && empty($_POST['year']) && empty($_GET ['year']))) {
    ...
    $data = file_get_contents("https://api.themoviedb.org/3/movie/upcoming?api_key=30abe6e1b3cd32a7e8d4b5ee6b117400&language=en-US&page=$pageNumber");
...
if (empty($_GET['page']) && (empty($_POST['sort_by'])) && empty($_GET['sort_by']) && empty($_POST['start']) && empty($_POST['end']) && empty($_GET['start']) && empty($_GET['end']) && empty($_POST['genres']) && empty($_POST['year']) && empty($_GET ['year'])) {
    $sort_key = "";
    $data = file_get_contents("https://api.themoviedb.org/3/movie/upcoming?api_key=30abe6e1b3cd32a7e8d4b5ee6b117400&language=en-US&page=1");
    $popularMovieArr = json_decode($data);
    $popularMovieArrResult = $popularMovieArr->results;
}
                        ...
                        <div id="collapseOne" class="accordion-collapse collapse show" aria-labelledby="headingOne" data-bs-parent="#accordionExample">
                            <div class="accordion-body">
                                ...
                                <select class="form-select" name="sort_by" aria-label="Default select example">
                                    <option value="popularity.desc" <?php echo $sort_key === "popularity.desc" ? "selected" : " " ?> <?php echo $sort_key === "" ? "selected" : " " ?> >Popularity Descending</option>
            ...
            init: function(options) {
                var o = $.extend({
                    ...
                    hrefTextSuffix:
                        ...
                        '<?php echo empty($_POST["sort_by"]) || empty($sort_key) ? "" : "&sort_by=$sort_key" ?><?php echo empty($start) ? "" : "&start=$start&end=$end" ?><?php echo empty($year) ? '' : "&year=$year" ?><?php echo empty($ans) ? "" : "&genres=$genresIdBeforeUrlCode" ?>'
                    ,
                    prevText: 'Prev',
                    nextText: 'Next',
                    ellipseText: '&hellip;',
